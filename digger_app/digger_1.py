...
def Call():
    ...
    thread = threading.Thread(target=getComments, args=(url,))
    thread.start()
    listbox.insert(END, 'Waiting...')
    print "Waiting..."
    thread.join()
    listbox.insert(END, 'Complete.')
    print "Complete."
...
listbox.insert(END, 'Please input douban book url, for example: http://book.douban.com/subject/1060852/')
